...
function App() {
  ...
  /* Фон стал розовым (см. ui_kits/website/theme.css), поэтому пятна тоже
     розовые: разные его оттенки плюс два акцента — жёлтый и тёмно-синий,
     ровно те, которыми на сайте покрашены плашки и элементы.
     Двигаются они теперь по оси Z (cs-drift-3d), то есть буквально
     приближаются и отдаляются. */
  const blobs = [
    { c: '#F9B2D2', x: '10%', y: '14%', s: '540px', t: '34s', d: '0s',   dx: '90px',  dy: '70px'  },
    { c: '#FCD8E9', x: '82%', y: '10%', s: '460px', t: '41s', d: '-8s',  dx: '-80px', dy: '90px'  },
    { c: '#EE4A9B', x: '70%', y: '52%', s: '600px', t: '47s', d: '-16s', dx: '70px',  dy: '-80px' },
    { c: '#FEE98A', x: '16%', y: '74%', s: '520px', t: '38s', d: '-24s', dx: '-70px', dy: '-60px' },
    { c: '#F47FB6', x: '46%', y: '88%', s: '440px', t: '44s', d: '-12s', dx: '60px',  dy: '-90px' },
    { c: '#1F6FBE', x: '90%', y: '62%', s: '380px', t: '52s', d: '-30s', dx: '-60px', dy: '70px'  },
  ];
  /* Кольца глубины: лежат «на полу» сцены на разном расстоянии от зрителя
     и медленно вращаются. Именно они читаются как объём, а не как заливка. */
  const rings = [
    { s: '620px',  t: '38s', d: '0s',   z: '-260px', o: 0.18 },
    { s: '900px',  t: '54s', d: '-12s', z: '-140px', o: 0.13 },
    { s: '1240px', t: '72s', d: '-26s', z: '-40px',  o: 0.09 },
  ];
  /* Шарики — тёмно-синие с жёлтыми: те же два цвета, которыми на сайте
     покрашены элементы и плашки, чтобы фон и контент читались как одно целое. */
  const balloons = [
    { c: '#10559B', x: '7%',  t: '26s', d: '-2s',  drift: '46px'  },
    { c: '#FBD130', x: '21%', t: '33s', d: '-14s', drift: '-32px' },
    { c: '#1F6FBE', x: '38%', t: '29s', d: '-23s', drift: '38px'  },
    { c: '#FBD130', x: '58%', t: '36s', d: '-7s',  drift: '-42px' },
    { c: '#0B4278', x: '74%', t: '30s', d: '-19s', drift: '30px'  },
    { c: '#1F6FBE', x: '89%', t: '38s', d: '-28s', drift: '-26px' },
  ];
  const clouds = [
  ...
  const backdrop = (
    <div className="cs-backdrop" aria-hidden="true">
      ...
      {rings.map((r, i) => (
        <span key={'r' + i} className="cs-depth-ring"
          style={{ '--s': r.s, '--t': r.t, '--d': r.d, '--z': r.z, '--o': r.o }} />
      ))}
